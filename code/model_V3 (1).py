...
path_for_trainset = "updated_dataset/train"
# path_for_testset = "updated_dataset/test"
path_for_validationset = "updated_dataset/validation"
...
train_generator = datagen.flow_from_directory(path_for_trainset,
                                              shuffle=False,
                                              frames_per_step=15,
                                              save_format='jpg',
                                              color_mode='rgb',
...
validation_generator = datagen.flow_from_directory(path_for_validationset,
                                                   shuffle=False,
                                                   frames_per_step=15,
                                                   save_format='jpg',
                                                   color_mode='rgb',
...
from keras.layers import Dense, Dropout, Activation, Flatten, Conv3D, MaxPooling3D,TimeDistributed,GRU,Bidirectional
...
def build_model(img_res):
    shape = (15, img_res[0], img_res[1], 3)
    model = Sequential()
    model.add(Conv3D(32,(3,5,5), strides=(1, 2, 2),kernel_initializer='random_uniform',bias_initializer=initializers.Constant(0.3), activation='relu',input_shape=shape))
    model.add(MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2)))
#     model.add(Dropout(0.5))
    model.add(Conv3D(64,(3,5,5),strides=(1, 2, 2), activation='relu',kernel_initializer='random_uniform',bias_initializer=initializers.Constant(0.3)))
    model.add(MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2)))
    model.add(Conv3D(128,(3,3,3), strides=(1, 1, 1), activation='relu',kernel_initializer='random_uniform',bias_initializer=initializers.Constant(0.3)))
    model.add(MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2)))
#     model.add(Dropout(0.5))
    ...
    model.add(Bidirectional(GRU(256,return_sequences=True,activation='tanh')))
    model.add(Flatten())
    model.add(Dense(1024, activation='relu',kernel_initializer='he_uniform',bias_initializer=initializers.Constant(0.3)))
...
def train_model(model,batch_size,epochs):
    history = model.fit_generator(train_generator,
            steps_per_epoch=59906//batch_size,
            epochs=epochs,
            shuffle=True,
            validation_steps=4785//batch_size,
...
if now.minute<10:
    time = str(now.hour+3)+":"+"0"+str(now.minute)
else:
    time = str(now.hour+3)+":"+str(now.minute)
print("~~~~~~~~~TIME AND DATE OF TRANING:", d1," ",time,"~~~~~~~~~")
model = build_model(img_res)
LR = 0.0001
compiled_model = compile_model(model,LR)
print()
get_model_summary(compiled_model)
epochs = 300
train_model(compiled_model,batch_size,epochs)
